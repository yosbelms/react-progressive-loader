...
export function Defer(props: HTMLAttributes<any> & {
  ...
  const { render, renderPlaceholder, loadOnScreen, ...rest } = props;
  const observedDivElProps = { ...rest, ...{ style: { ...observedElStyle, ...rest.style } } };
          ...
          <When test={true} render={() =>
            <div {...observedDivElProps}
              ...
              <If test={cmp.state.loading} then={() =>
                (loadComponent(cmp, props), renderPlaceholder && renderPlaceholder())
              } />
            </div>
          } />
        </Switch>
...
function loadComponent(cmp, props) {
  ...
  if (result.then) {
    result.then((mod) => {
      ...
      if (mod) {
        if (typeof mod === 'function' || React.isValidElement(mod)) {
          cmpToRender = mod;
        } else if (typeof mod.default === 'function' || React.isValidElement(mod.default)) {
